import { drawMeadow } from './illustratedTerrain.js'
import { isTilePurchasable, getTileCost } from '#game/engine/landEngine.js'

const TILE_LOCKED = '#2a2a2a'
const TILE_UNLOCKED = '#3d4a3a'
const TILE_BORDER = '#759c5e40'
const LOCKED_BORDER = '#89b47520'
const TILE_PURCHASABLE = '#f4df91'
const PURCHASABLE_BORDER = '#a37f32'
 ...
 */
export function drawGrid(ctx, { maxRegion, ownedTileSet, tileSize, camera, expandMode = false, maxPurchasableRing = 0, motionTime = 0, showGrid = false, width = Infinity, height = Infinity }) {
  for (let gx = maxRegion.x0; gx <= maxRegion.x1; gx++) {
    for (let gy = maxRegion.y0; gy <= maxRegion.y1; gy++) {
      ...
      if (px + size < 0 || py + size < 0 || px > width || py > height) continue

      if (isOwned) {
        drawMeadow(ctx, gx, gy, px, py, size, true, motionTime)
        ctx.strokeStyle = showGrid ? '#618149' : TILE_BORDER
        ctx.lineWidth = 1
        ...
        drawMeadow(ctx, gx, gy, px, py, size, false, motionTime)
        ctx.strokeStyle = LOCKED_BORDER
        ctx.lineWidth = 1
      }
      ctx.strokeRect(px + 0.5, py + 0.5, size - 1, size - 1)
      if (isOwned) {
        ctx.strokeStyle = '#f7f5d4'
        ctx.lineWidth = 3
        ctx.beginPath()
        if (!ownedTileSet.has(`${gx},${gy - 1}`)) { ctx.moveTo(px, py); ctx.lineTo(px + size, py) }
        if (!ownedTileSet.has(`${gx},${gy + 1}`)) { ctx.moveTo(px, py + size); ctx.lineTo(px + size, py + size) }
        if (!ownedTileSet.has(`${gx - 1},${gy}`)) { ctx.moveTo(px, py); ctx.lineTo(px, py + size) }
        if (!ownedTileSet.has(`${gx + 1},${gy}`)) { ctx.moveTo(px + size, py); ctx.lineTo(px + size, py + size) }
        ctx.stroke()
      }
    }
  }
