  ...
  <div class="market-backdrop" @click.self="emit('close')">
    <section ref="dialog" class="market-panel" role="dialog" aria-modal="true" aria-labelledby="market-title" tabindex="-1" @keydown="keys">
      ...
      <article v-for="order in board.orders" :key="order.id" class="order" :class="{ fulfilled: order.completed }" :aria-labelledby="`order-${order.id}`">
        ...
        <div class="progress-label"><span>{{ order.completed ? 'Order fulfilled' : `${progress(order).delivered} / ${progress(order).total} delivered` }}</span><span>{{ progress(order).percent }}%</span></div>
        <progress :value="progress(order).delivered" :max="progress(order).total" :aria-label="`${MARKET_CUSTOMERS[order.customer].name}'s order progress`"></progress>
        <button class="deliver" :disabled="order.completed || progress(order).available <= 0" @click="deliver(order)">{{ order.completed ? 'Completed' : progress(order).available > 0 ? `Deliver ${progress(order).available} cigars` : 'Waiting for cigars' }}</button>
      </article>
    </section>
  </div>
</template>
...
h2, h3, p { margin: 0; } h2 { font-size: 1.5rem; } h3 { font-size: 1.15rem; }
...
.order { border-top: 1px solid $color-panel-border; padding: $spacing-md 0; margin-top: $spacing-md; }
.customer { justify-content: flex-start; } .reward { margin-left: auto; color: $color-money; font-size: 1.25rem; text-align: right; white-space: nowrap; } small { display: block; font-weight: normal; margin-top: 3px; }
.difficulty { display: inline-block; color: $color-accent; margin-top: 6px; font-size: 0.8rem; }
.requirements { list-style: none; padding: 0; margin: $spacing-sm 0 $spacing-md; }
li { display: flex; align-items: center; gap: $spacing-sm; padding: $spacing-sm 0; } li strong { margin-left: auto; white-space: nowrap; font-variant-numeric: tabular-nums; }
.swatch { width: 14px; height: 14px; flex-shrink: 0; border-radius: 3px; }
.progress-label { color: $color-text-muted; font-size: 0.8rem; }
progress { width: 100%; height: 10px; display: block; margin: $spacing-sm 0 $spacing-md; accent-color: $color-money; border: 0; border-radius: 5px; overflow: hidden; background: $color-bg; }
...
.deliver { width: 100%; background: $color-accent; color: $color-bg; border: 0; font-weight: bold; }
...
@include mobile { .market-panel { padding: $spacing-md; } .customer { flex-wrap: wrap; } .customer :deep(canvas) { width: 70px; height: 70px; } .reward { font-size: 1.1rem; } }
</style>
